...
namespace ClosedXML.Excel
{
    ...
    internal class XLTable : XLRange, IXLTable
    {
        ...
        internal void AddFields(IEnumerable<ColumnDataType> columnDataTypes)
        {
            _fieldNames = new Dictionary<String, IXLTableField>();

            Int32 cellPos = 0;
            foreach (var field in columnDataTypes)
            {
                _fieldNames.Add(field.Name, new XLTableField(this, field.Name) { Index = cellPos++, ColumnDataType = field });
            }
        }
        ...
        public ExcelDataTable AsExcelDataTable()
        {
            var table = new ExcelDataTable(Name);
            ...
            foreach (var f in Fields.Cast<XLTableField>())
            {
                ...
                var dc = table.Columns.Add(f.Name, type);
                table.ColumnDataTypes.Add(dc, f.ColumnDataType);
            }
